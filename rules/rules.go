...
var (
	deleteRuleURL  = "http://%s/api/%s/rules/%d"
	createRuleURL  = "http://%s/api/%s/rules"
	getAllRulesURL = "http://%s/api/%s/rules"
	getRuleURL     = "http://%s/api/%s/rules/%d"
)
...
type Rule struct {
	...
	Periodic       int16       `json:"periodic,omitempty"`
...
func (l *Rules) GetRule(ruleID int) (Rule, error) {
	var ll Rule
	url := fmt.Sprintf(getRuleURL, l.Hostname, l.Username, ruleID)
...
func (l *Rules) CreateRule(rule Rule) ([]conbee.ApiResponse, error) {
...
func (l *Rules) GetAllRules() ([]Rule, error) {
	url := fmt.Sprintf(getAllRulesURL, l.Hostname, l.Username)
	request, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return nil, err
	}
